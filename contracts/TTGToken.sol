...
contract DistributeToken is StandardToken, Ownable{
  ...
  event CrowDistribute(address from, address to, uint amount);
  ...
  uint public maxAirDrop = 1000;//need below 1000 TTG

  uint public havedAirDrop = 0;
  uint public totalAirDrop = 0; //totalSupply * 5%

  bool public finishCrowCoin = false;
  uint public havedCrowCoin = 0;
  uint public totalCrowCoin = 0; //totalSupply * 50%
  ...
  /* Withdraw */
  /*
    NOTICE: These functions withdraw the ETH which remained in the contract account when user call crowDistribute
  */
 ...
 function airDrop(address receiver, uint amount) onlyDistributeAgent  public  returns (uint actual){
 ...
 function crowDistribute() payable public  returns (uint actual) {
      
    require(msg.sender != address(0));
    require(!isContract(msg.sender));
    require(msg.value != 0);
    require(totalCrowCoin < havedCrowCoin);
    require(finishCrowCoin == false);
    
    uint actualAmount = calculateCrowAmount(msg.value);

    require(actualAmount != 0);

    havedCrowCoin = havedCrowCoin.plus(actualAmount);
    ...
    // This will make the mint transaction apper in EtherScan.io
    // We can remove this after there is a standardized minting event
    emit CrowDistribute(0, msg.sender, actualAmount);

    return actualAmount;
  }

 function  switchCrowState () internal{

    if (havedCrowCoin < totalCrowCoin.mul(10).div(100) ){
       crowState = 0;

    }else  if (havedCrowCoin < totalCrowCoin.mul(20).div(100) ){
       crowState = 1;
    
    } else if (havedCrowCoin < totalCrowCoin.mul(30).div(100) ){
       crowState = 2;

    } else if (havedCrowCoin < totalCrowCoin.mul(40).div(100) ){
       crowState = 3;

    } else if (havedCrowCoin < totalCrowCoin.mul(50).div(100) ){
       crowState = 4;
    }
      
    if (havedCrowCoin >= totalCrowCoin) {
       finishCrowCoin = true;
  }
 }

function calculateCrowAmount (uint _price) internal view returns (uint _crow) {
        
    if (crowState == 0) {
      return _price.mul(100000);
    }
      ...
      return  _price.mul(25000);

    } else if (crowState == 3) {
     return  _price.mul(12500);

    } else if (crowState == 4) {
     return  _price.mul(6250);
    }
...
contract ReleasableToken is ERC20, Ownable {
  ...
  uint private maxTransferForDev  = 40000000;
  uint private maxTransferFoFounds= 20000000;
 ...
 *
 */
contract TTGToken is ReleasableToken, MintableToken , DistributeToken, RecycleToken{
  ...
  function TTGToken() public {
    ...
    totalCrowCoin = totalSupply.mul(50).div(100);
